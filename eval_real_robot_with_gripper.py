...
from demo_real_robot import transform_point
...
@click.command()
@click.option('--input', '-i', default="data/outputs/blah/checkpoints/pour_water_r34.ckpt", required=True, help='Path to checkpoint')
...
@click.option('--output', '-o', default="data/eval_pusht_real", required=True, help='Path to checkpoint')
        ...
        with Spacemouse(shm_manager=shm_manager) as sm, \
            ...
            while True:
                ...
                while True:
                    ...
                    target_pose[:3]=transform_point(target_pose[:3], transformation_matrix)
                    target_pose[:3] = np.clip(target_pose[:3], [0.18, -0.51, -0.14], [0.72, -0.26, 0.53])
                    target_pose[:3]=transform_point(target_pose[:3], inverse_transformation_matrix)
                    # , 0.72], [-0.51,-0.26], [-0.14, 0.53])
                ...
                try:
                    ...
                    while True:
                        ...
                        # this_target_poses_temp = transform_point(this_target_poses[:,:2], transformation_matrix)
                        # clip actions
                        # this_target_poses[:,:2] = np.clip(
                        #     this_target_poses[:,:2], [0.25, -0.45], [0.77, 0.40])
                        # for i in range(len(this_target_poses)):
                        #     this_target_poses[i,:3]=transform_point(this_target_poses[i,:3], transformation_matrix)
                        #     this_target_poses[i,:3] = np.clip(this_target_poses[i,:3], [0.18, -0.51, -0.14], [0.72, -0.26, 0.53])
                        #     this_target_poses[i,:3]=transform_point(this_target_poses[i,:3], inverse_transformation_matrix)
                        # this_target_poses[:,:3]=transform_point(this_target_poses[:,:3], transformation_matrix)
                        # this_target_poses[:,:3] = np.clip(this_target_poses[:,:3], [0.18, -0.51, -0.14], [0.72, -0.26, 0.53])
                        # this_target_poses[:,:3]=transform_point(this_target_poses[:,:3], inverse_transformation_matrix)
                        ...
                        gripper_state_now = action[-1][6]
                        if gripper_state_now != gripper_state_last:
                            if gripper_state_now == 1:
                                print("打开夹爪")
                                gripper.moveMax_cmd(200) # 发送夹爪打开指令
                            else:
                                print("关闭夹爪")
                                gripper.moveMinHold_cmd(200, 100) # 发送夹爪关闭指令
                        gripper_state_last = gripper_state_now
                        # visualize
                        episode_id = env.replay_buffer.n_episodes
                        vis_img = obs[f'camera_{vis_camera_idx}'][-1]
